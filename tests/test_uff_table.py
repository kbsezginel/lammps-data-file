...
from lammps_data.uff_table import UffTable, \
                                  NonExistentElementError, \
                                  NonExistentCoordinationError, \
                                  NonExistentAngleError, \
...
def test_lookup_of_matching_angle_should_return_the_row(uff_table):
    expected_row = dict(type='C_1', element='C', bond=0.706, angle=180.0, distance=3.851,
                        energy=0.105, scale=12.73, charge=1.912, coordination=2)
...
def test_lookup_with_angle_multiplicity_using_high_tolerance_should_raise_an_exception(uff_table):
